...
本文档详细记录了在 LLaMA-Factory 框架中实现样本级损失加权功能的完整技术过程，包括代码追踪、修改点说明及实现细节。全文按照 SFT 微调的数据流顺序组织，读者可沿着 `run_sft` 入口逐步追踪 `loss_weight` 从数据集配置到损失计算的完整传递链路。
...
在所有单个数据集完成加载与对齐后，`merge_dataset` 将它们融合为一个统一的 Dataset 对象。经过此步骤，数据集中已包含 `_loss_weight` 字段（由代码修改2注入），为后续预处理阶段传递权重信息做好了准备。
...
当启用 packing 时，多条样本会被打包到同一个序列中以提高训练效率。因此 `loss_weight` 也需要相应地被打包。
...
在数据集准备完毕后，`run_sft` 初始化 `CustomSeq2SeqTrainer` 实例，将模型、数据集、分词器等组件组装到一起，为后续训练执行做好准备。
...
`get_train_dataloader` 将训练 Dataset 包装为 PyTorch DataLoader，使其可以按批次迭代输出。在此过程中，一个关键步骤是 `_remove_unused_columns`，它会根据模型的 forward 方法签名过滤掉数据集中不需要的列。**对于 `loss_weight` 字段，必须确保它不会被过滤掉**，这通过代码修改5实现。
...
### loss_weight 完整传递链路总结

`loss_weight` 从数据配置到最终损失计算的完整传递路径如下：

1. **数据集配置**：用户在 `dataset_info.json` 的 `columns` 中声明 `"loss_weight": "loss_weight"`，在训练数据中为每个样本设置 `loss_weight` 值（代码修改1）
2. **数据对齐**：`convert_sharegpt` 将原始 `loss_weight` 映射为标准字段 `_loss_weight`（代码修改2）
3. **数据预处理**：`preprocess_supervised_dataset` 或 `preprocess_packed_supervised_dataset` 将 `_loss_weight` 传递为 `loss_weight` 字段（代码修改3、代码修改4）
4. **列保留**：`_set_signature_columns_if_needed` 重写确保 `loss_weight` 不被 `_remove_unused_columns` 过滤（代码修改5）
5. **损失计算**：重写的 `compute_loss` 从 inputs 中取出 `loss_weight`，通过 `label_smoother_weighted` 实现样本级加权（代码修改6）
