...
本文档记录了在 LLaMA-Factory 框架中实现**样本级损失加权**功能的完整技术过程。为便于学习和检索，全文按照**数据流顺序**组织——从一条训练数据在磁盘上的原始形态出发，依次经过数据加载、格式对齐、Tokenization 预处理、DataLoader 整理、列过滤，最终到达模型前向传播与损失计算，完整追踪 `loss_weight` 字段在每一阶段的传递与变换。
...
训练数据以 JSON 数组的形式存储，每个元素是一个样本。在标准格式的基础上，我们为每个样本增加 `loss_weight` 字段（与 `messages` 平级），默认值为 `1`。
