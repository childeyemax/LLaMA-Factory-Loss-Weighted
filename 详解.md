...
**补充说明**
- CustomSeq2SeqTrainer是Trainer的子类(Seq2SeqTrainer)的子类，self.train的定义直接来自于Trainer类
- Trainer._inner_training_loop 方法将dataset变为inputs
- inputs被用来直接计算loss
&nbsp;
...
get_dataset函数：<https://github.com/hiyouga/LLaMA-Factory/blob/v0.9.1/src/llamafactory/data/loader.py#L225>
...
def get_dataset(
    ...
    # 2. 预处理数据集 —— 见SupervisedDatasetProcessor.preprocess_dataset方法的返回值
...
### 核心步骤
1. 建立DatasetAttr实例列表 `get_dataset_list(dataset_names, data_args.dataset_dir)`
2. 利用DatasetAttr实例中的信息加载、对齐数据集 `datasets.append(_load_single_dataset(dataset_attr, model_args, data_args, training_args))`
3. 融合数据集 `merge_dataset(datasets, data_args, seed=training_args.seed)`

_get_merged_dataset函数：<https://github.com/hiyouga/LLaMA-Factory/blob/v0.9.1/src/llamafactory/data/loader.py#L153>
...
### 1.1.1 建立DatasetAttr实例列表
`get_dataset_list(dataset_names, data_args.dataset_dir)`

读取数据集文件，返回DatasetAttr对象(每个对应一个数据集)构成的列表
...
DatasetAttr的定义：<https://github.com/hiyouga/LLaMA-Factory/blob/v0.9.1/src/llamafactory/data/parser.py#L26>
...
1. 在dataset_info的"columns"中增加 `"loss_weight": "loss_weight"`（与"messages"平级）；
2. 数据集中增加 "loss_weight": 1(默认值)，注意与"messages"平级；
3. DatasetAttr：初始化时增加语句：`loss_weight: Optional[float] = None`, 并在get_dataset_list函数中将对应代码替换为
```python
if "columns" in dataset_info[name]:
...
#### sharegpt格式 & dataset_info.json中的数据集描述
```json
"数据集名称": {
  "file_name": "data.json",
  "formatting": "sharegpt",
...
### 1.1.2 利用DatasetAttr实例中的信息加载、对齐数据集 
`datasets.append(_load_single_dataset(dataset_attr, model_args, data_args, training_args))`
...
align_dataset函数：<https://github.com/hiyouga/LLaMA-Factory/blob/v0.9.1/src/llamafactory/data/aligner.py#L230>

convert_sharegpt函数：<https://github.com/hiyouga/LLaMA-Factory/blob/v0.9.1/src/llamafactory/data/aligner.py#L137>
...
##### 代码修改 (step 2)
convert_sharegpt函数的output字典中增加：`"_loss_weight": example[dataset_attr.loss_weight] if dataset_attr.loss_weight else None,`

注意，如果把上面的None换成1，可能导致错误难以发觉！
...
### 核心步骤
1. _get_preprocessed_dataset函数利用preprocess_func函数对dataset中的每个样本进行处理；
2. 在sft阶段，preprocess_func函数是preprocess_supervised_dataset或者preprocess_packed_supervised_dataset函数的偏函数

_get_preprocessed_dataset函数：<https://github.com/hiyouga/LLaMA-Factory/blob/v0.9.1/src/llamafactory/data/loader.py#L176>

get_preprocess_and_print_func函数被调用返回preprocess_func函数：<https://github.com/hiyouga/LLaMA-Factory/blob/v0.9.1/src/llamafactory/data/preprocess.py#L36>
...
### preprocess_supervised_dataset函数
...
对"preprocess_supervised_dataset"函数增加语句：`model_inputs["loss_weight"].append(examples["_loss_weight"][i])`
...
## Trainer, Seq2SeqTrainer, CustomSeq2SeqTrainer的初始化
...
## 注意事项
1. CustomSeq2SeqTrainer继承自Seq2SeqTrainer，Seq2SeqTrainer继承自Trainer（后面两个来自transformers库）;
2. train方法直接来自Trainer;
3. 如果`compute_loss_func=None`，那么compute_loss方法默认使用LabelSmoother的__call__方法计算损失函数；
4. CustomSeq2SeqTrainer在Trainer的基础上改写了compute_loss方法。
...
```
2. inner_training_loop为self._inner_training_loop方法的偏函数：`inner_training_loop = find_executable_batch_size(self._inner_training_loop, self._train_batch_size, args.auto_find_batch_size)`，参考find_executable_batch_size函数：<https://github.com/huggingface/transformers/blob/v4.46.1/src/transformers/trainer_utils.py#L764>；
3. self._inner_training_loop方法调用self.training_step：`tr_loss_step = self.training_step(model, inputs, num_items_in_batch)`
4. self.training_step调用self.compute_loss；
5. self.compute_loss默认通过LabelSmoother类的__call__方法计算损失函数；
6. CustomSeq2SeqTrainer在Trainer的基础上改写了compute_loss方法。

## 3.1 Train.train方法

训练过程由trainer.train方法执行，其定义来自于Trainer类：<https://github.com/huggingface/transformers/blob/v4.46.1/src/transformers/trainer.py#L2021>
...
## 3.2 Trainer._inner_training_loop方法
...
4. batch_samples是迭代器epoch_iterator的元素组成的列表；
	- `batch_samples, num_items_in_batch = self.get_batch_samples(epoch_iterator, num_batches)`
	- self.get_batch_samples方法的源码见：<https://github.com/huggingface/transformers/blob/v4.46.1/src/transformers/trainer.py#L5033>
5. inputs是batch_samples的一个元素。

### 支线：Trainer.get_train_dataloader方法

<https://github.com/huggingface/transformers/blob/v4.46.1/src/transformers/trainer.py#L942>

**注意**
DataLoader是torch.utils.data库的函数
...
Trainer._remove_unused_columns方法(<https://github.com/huggingface/transformers/blob/v4.46.1/src/transformers/trainer.py#L865>)的完整代码如下：
...
Trainer.self._set_signature_columns_if_needed方法(<https://github.com/huggingface/transformers/blob/v4.46.1/src/transformers/trainer.py#L850>)全部代码如下：
...
在CustomSeq2Seq中重写_set_signature_columns_if_needed方法，保留"loss_weight"，此修改放在文章最后。


## 3.3 Trainer.training_step方法
...
### 支线：Trainer._prepare_inputs方法 & Trainer._prepare_input方法
...
Trainer.training_step调用了`inputs = self._prepare_inputs(inputs)`，Trainer._prepare_inputs中调用了`inputs = self._prepare_input(inputs)`。
它们的完整源码如下。

可以看出：
1. Trainer._prepare_input方法首先将输入的inputs中的torch.tensor转移到self.args.device设备中，如果采用deepseed还要把浮点数和复数的数格式转化为self.accelerator.state.deepspeed_plugin.hf_ds_config.dtype()；
2. Trainer._prepare_inputs方法给inputs增加新的键值对：`inputs["mems"] = self._past`。
...
## 3.6 CustomSeq2SeqTrainer在Trainer的基础上改写了compute_loss方法
...
### 1. 重写CustomSeq2SeqTrainer的compute_loss方法

**注意：**
1. 某些大模型的forward方法可能需要输入labels，则将下面的对应代码替换为：`outputs = model(**inputs, labels=labels)`;
...
### 2. 新增CustomSeq2SeqTrainer的label_smoother_weighted方法

改编自LabelSmoother类的__call__方法，方便重写的compute_loss方法调用。
...
### 3. 重写CustomSeq2SeqTrainer的_set_signature_columns_if_needed方法
